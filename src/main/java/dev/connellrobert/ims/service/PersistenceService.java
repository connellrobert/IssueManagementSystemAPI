...
public class PersistenceService {
	...
	public IMSUser saveUser(IMSUser u) {
	return userRepo.save(u);	
	}
	
	
	public UserProjectDTO SaveUser(IMSUser u) {
		if(u.getProjects() != null) {
			IMSUser IMSUser = uPRepo.save(u.getProjects().iterator().next()).getUser();
			return conversion.getUserProjectConversion().apply(IMSUser);
		}
	...
	public UserProjectDTO subscribeUser(UserSubscribeDTO dto) {
		IMSUser u = userRepo.findByUsername(dto.getUsername()).orElseThrow(() -> new IllegalArgumentException());
